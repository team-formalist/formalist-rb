...
RSpec.describe Formalist::Form do
  ...
  subject(:form_class) {
    ...
    Class.new(Formalist::Form) do
      define do
        many_forms :components, label: "Components", embeddable_forms: embedded_forms
  ...
  it "outputs an AST" do
    ...
    expect(form.to_ast[0][1][4]).to eq([{
      :name=>:image_with_caption,
      :label=>"Image with caption",
      :data=>{:image_id=>1234, :caption=>"Cute cat"},
      :form=>[[:field, [:image_id, :field, 1234, [], [:object, []]]], [:field, [:caption, :field, "Cute cat", [], [:object, []]]]]}]
    )
  end
end
